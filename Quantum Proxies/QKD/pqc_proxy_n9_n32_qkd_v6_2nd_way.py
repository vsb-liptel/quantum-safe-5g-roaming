...
KME_UDP_EGRESS_HOST = "10.100.2.204"
...
# TCP1 backend
KME_TCP1_BACKEND_HOST = "10.10.1.251"
KME_TCP1_BACKEND_PORT = 7777

# TCP2 backend
KME_TCP2_BACKEND_HOST = "10.10.1.252"
...
def _parse_switch_tag(tag: str) -> Tuple[int, int, int]:
    """
    tag is a 3-digit string: TCP1 TCP2 UDP
    digits in {'1','2','3'}
    UDP supports only 1/2; digit 3 is treated as 1 for UDP.
    """
...
class SaeMeasure:
    """
    Records per-second newly buffered bits into SAE pool.
    Rows:
      time_sec_since_start, newly_buffered_bits
    Row 0 is always 0.
    Row t represents interval (t-1, t].
    """
    def __init__(self, out_csv_path: str, key_size_bits: int, multiplier: int = 1) -> None:
        ...
        self._bits_by_sec: Dict[int, int] = {}  # sec -> bits added in that second (t>=1)

    def _event_sec(self) -> int:
        # Put event at exact boundary t=1.0 into second 1, not 2
        elapsed = time.time() - self.start_ts
        sec = int(math.ceil(elapsed))
        return max(1, sec)
    ...
    def record_keys_inserted(self, inserted_keys: int) -> None:
        ...
        sec = self._event_sec()
        with self._lock:
            self._bits_by_sec[sec] = self._bits_by_sec.get(sec, 0) + bits

    def pop_bits_for_sec(self, sec: int) -> int:
        with self._lock:
            return self._bits_by_sec.pop(sec, 0)

    def run_writer(self, stop_evt: threading.Event) -> None:
        os.makedirs(os.path.dirname(self.out_csv_path) or ".", exist_ok=True)
        with open(self.out_csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["time_sec", "newly_buffered_bits"])
            w.writerow([0, 0])
            f.flush()
            ...
            while not stop_evt.is_set():
                elapsed = time.time() - self.start_ts
                completed = int(elapsed)  # seconds fully completed
                # Write rows 1..completed
                for sec in range(last_written + 1, completed + 1):
                    w.writerow([sec, self.pop_bits_for_sec(sec)])
                    f.flush()
                    last_written = sec
                stop_evt.wait(0.2)

            # On stop: flush up to current (possibly partial) second index
            elapsed = time.time() - self.start_ts
...
def _parse_json_bytes(raw: bytes) -> dict:
    ...
    # Only gunzip if it's truly gzip (some endpoints mislabel encoding)
...
def kms_fetch_enc_keys() -> List[Tuple[str, str]]:
    url = kms_enc_url()
    _log("KMS", f"ENC URL: {url}")

    # Force identity; also ensure urllib3 doesn't try to decode
    ...
    for k in obj.get("keys", []):
        kid = k.get("key_ID", "")
        kb64 = k.get("key", "")
        if not kid or not kb64:
            continue
        out.append((kid, _bytes_to_hex(_b64_to_bytes(kb64))))
    return out

def kms_fetch_dec_keys(key_ids: List[str]) -> Dict[str, str]:
    url = kms_dec_url()
    _log("KMS", f"DEC URL: {url}")
    ...
    for k in obj.get("keys", []):
        ...
        if not kid or not kb64:
            continue
        out[kid] = _bytes_to_hex(_b64_to_bytes(kb64))
    return out
...
class SaeKeyPool:
    def __init__(self) -> None:
        self._fifo: Deque[Tuple[str, str]] = deque()  # (key_id, psk_hex)
        self._lock = threading.Lock()
    ...
    def push_many_capped(self, keys: List[Tuple[str, str]], cap: Optional[int]) -> List[Tuple[str, str]]:
        ...
        with self._lock:
            for k in keys:
                if cap is not None and len(self._fifo) >= cap:
                    break
                self._fifo.append(k)
                inserted.append(k)
        return inserted
        ...
        try:
            if state.pool.size() < SAE_POOL_LOW_WATER:
                ...
                if keys:
                    ...
                    if inserted:
                        _log("SAE-KMS", f"Prefetch: added={len(inserted)}, sae_pool={state.pool.size()} (target={SAE_POOL_TARGET}, low={SAE_POOL_LOW_WATER})")

                        if state.meas is not None:
                            state.meas.record_keys_inserted(len(inserted))

                        # Prefetch sync to KME only for inserted IDs (others were dropped due to cap)
                        payload = {"op": "prefetch", "key_IDs": [{"key_ID": kid} for (kid, _) in inserted]}
                        if keysync_post(payload):
                            _log("SAE-KMS", f"Key_IDs synced to KME: {', '.join([kid for (kid, _) in inserted])}")
                    else:
                        _log("SAE-KMS", f"Prefetch: pool capped at {SAE_POOL_TARGET}; inserted=0 (sae_pool={state.pool.size()})")
                else:
                    _log("SAE-KMS", "Prefetch: KMS returned no keys")
        except Exception as e:
            _log("SAE-KMS", f"Prefetch exception: {repr(e)}")

        state.stop.wait(SAE_KMS_POLL_SEC)

def _sae_commit_initial_service(service: str, kid: str, port_a: int, port_b: int) -> bool:
    """
    Proper initial activation so KME binds ports early.
    Commit new_port=port_a and retire port_b with grace=0 (no real retire).
    """
    if not keysync_post({"op": "prepare", "service": service, "key_ID": kid, "port": port_a}):
        return False
...
def _sae_rotate_service(state: SaeState, svc: SaeServiceState, grace: float) -> bool:
    ...
    if not km:
        _log("SAE-KMS", f"{svc.name.upper()} Rotate: no key available; sae_pool={state.pool.size()} (low={SAE_POOL_LOW_WATER})")
        return False
    ...
    if not keysync_post({"op": "prepare", "service": svc.name, "key_ID": new_kid, "port": new_port}):
        _log("SAE-KMS", f"{svc.name.upper()} prepare failed; keeping current key")
    ...
    if not keysync_post({"op": "commit", "service": svc.name, "key_ID": new_kid, "new_port": new_port, "old_port": old_port, "grace": grace}):
        _log("SAE-KMS", f"{svc.name.upper()} commit failed; keeping current key")
        state.pool.push_front(km)
        return False
...
def sae_udp_restart_client(state: SaeState) -> None:
    with state._udp_client_lock:
        ...
        if not state.udp.key_id or not state.udp.psk_hex:
            return
        ...
        while time.time() < deadline and not state.stop.is_set():
            ...
            probe_deadline = time.time() + 0.005  # 5 ms probe
            while p.poll() is None and time.time() < probe_deadline:
                time.sleep(0.001)
            if p.poll() is None:
                break
...
def sae_udp_proxy_loop(state: SaeState) -> None:
    ...
    _log("SAE-UDP", f"Listening on {(SAE_UDP_LISTEN_HOST, SAE_UDP_LISTEN_PORT)}")
    ...
    while not state.stop.is_set():
        try:
            ...
            if not state.udp.key_id:
                continue

            # payload accounting (mode 2)
            if state.udp_mode == 2:
...
def sae_udp_rotator(state: SaeState) -> None:
    ...
    while not state.stop.is_set():
        if state.udp_mode == 1:
            ...
            # mode 2: (time OR payload), whichever happens first
            ...
            while not state.stop.is_set():
                ...
                if state.stop.is_set():
                    break
                if not state.udp.rotate_req.is_set():
                    continue
                state.udp.rotate_req.clear()
                if _sae_rotate_service(state, state.udp, RETIRE_GRACE_SEC_UDP):
                    sae_udp_restart_client(state)
...
def _sae_tcp_listener_loop(
    ...
    ls.listen(50)
    _log("SAE-TCP", f"{svc.name} listening on {(listen_host, listen_port)} -> TLS to {KME_HOST}:{tls_port_a}/{tls_port_b} ({backend_label})")
    ...
    while not state.stop.is_set():
        try:
            ...
            # --- OPTIMIZATION: Disable Nagle's Algorithm for SAE TCP socket ---
            cs.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            # ------------------------------------------------------------------

            # Session-based mode: switch key at start of each new session (assumes no overlaps)
            if mode == 3:
                if not _sae_rotate_service(state, svc, RETIRE_GRACE_SEC_TCP):
                    _log("SAE-TCP", f"{svc.name} no key available; dropping client {caddr}")
                    cs.close()
                    continue

            if not svc.key_id or not svc.psk_hex:
                _log("SAE-TCP", f"{svc.name} no PSK yet; dropping client {caddr}")
                cs.close()
                continue
            ...
            # Retry TLS connect briefly (KME might be starting listener right now)
            deadline = time.time() + SAE_TCP_TLS_CONNECT_RETRY_SEC
            p: Optional[subprocess.Popen] = None
            while time.time() < deadline and not state.stop.is_set():
                p = _openssl_s_client_psk(KME_HOST, port, psk, kid)
                probe_deadline = time.time() + 0.005  # 5 ms
                while p.poll() is None and time.time() < probe_deadline:
                    time.sleep(0.001)
                if p.poll() is None:
                    break
            ...
            if p is None or p.stdin is None or p.stdout is None:
                _log("SAE-TCP", f"{svc.name} TLS connect failed, dropping client {caddr}")
                cs.close()
                continue

            _log("SAE-TCP", f"{svc.name} client {caddr} -> TLS {KME_HOST}:{port} key_ID={kid}")
            ...
            cs.close()
            _kill_proc(p, f"{svc.name}_s_client")
            _log("SAE-TCP", f"{svc.name} client {caddr} closed")

            # Payload-based mode: rotate after threshold, but only affects next session.
            # Mode 2 is now "time OR payload": time-rotator may rotate during/after session.
            if mode == 2:
                # Only count bytes if key didn't change during this session
                if svc.key_id == kid:
                    svc.bytes_since_rotate += local_bytes
                    if svc.bytes_since_rotate >= switch_bytes:
                        svc.bytes_since_rotate = 0
                        _log("SAE-KMS", f"{svc.name.upper()} payload threshold reached; rotating for next session")
                        _sae_rotate_service(state, svc, RETIRE_GRACE_SEC_TCP)
                else:
                    # Time-based rotation already happened -> discard bytes for old key
                    svc.bytes_since_rotate = 0
...
def _sae_tcp_time_rotator(state: SaeState, svc: SaeServiceState, refresh_sec: float, mode: int) -> None:
    """
    Time-based rotation worker.
    Mode 1: time-only
    Mode 2: time OR payload (payload handled in listener); time still active here
    Mode 3: per-session (no periodic time rotation)
    """
    if mode not in (1, 2):
        return

    # Initial activation so KME binds ports early (nice for first client)
    while not state.stop.is_set() and svc.key_id is None:
        ...
        svc.bytes_since_rotate = 0
        _log("SAE-KMS", f"{svc.name.upper()} Activated key_ID={kid}; sae_pool={state.pool.size()} (low={SAE_POOL_LOW_WATER})")

        if not _sae_commit_initial_service(svc.name, kid, svc.port_a, svc.port_b):
            _log("SAE-KMS", f"{svc.name.upper()} initial commit failed; will retry shortly")
            state.pool.push_front((kid, psk))
            svc.key_id = None
            svc.psk_hex = None
            state.stop.wait(1.0)
...
def run_sae(tag: str, out_csv: str) -> None:
    ...
    # Use the config variable to set multiplier
    ...
    threads = [
        threading.Thread(target=meas.run_writer, args=(st.stop,), daemon=True),
    ...
    _log("MAIN", f"SAE running (UDP + TCP1 + TCP2). switch_tag={tag} (tcp1={tcp1_mode}, tcp2={tcp2_mode}, udp={udp_mode}). Ctrl+C to stop.")
    _log("MEASURE", f"Writing newly-buffered-bits CSV to: {out_csv} (key_size_bits={KMS_KEY_SIZE}, multiplier={meas.multiplier})")
...
def kme_prefetch_worker(st: KmeState) -> None:
    while not st.stop.is_set():
        ...
        try:
            ...
            added = st.keys.add_many(kv)
            _log("KME-KMS", f"Prefetch: added={added}, kme_buf={st.keys.size()} (target~{SAE_POOL_TARGET})")
        except Exception as e:
            _log("KME-KMS", f"Prefetch exception: {repr(e)}")
...
def _kme_ensure_dec_key(st: KmeState, kid: str) -> bool:
    ...
    try:
        ...
        _log("KME-KMS", f"Ensure dec_key failed for {kid}: {repr(e)}")
        return False
    ...
    while not st.stop.is_set():
        with ctx.lock:
            if ctx.retire_deadline and time.time() >= ctx.retire_deadline:
                _log("KME-KMS", f"UDP Commit: retired old port={ctx.retire_port} after grace")
        ...
        try:
            ...
            _log("KME", f"UDP TLS listener up on :{port}")
            while not st.stop.is_set():
                if proc.poll() is not None:
                    _drain_stderr(proc, "KME-OPENSSL")
                    break
...
def _kme_ctx_tcp_port_kid(ctx: KmeServiceCtx, fixed_port: int, label: str) -> Optional[str]:
    ...
    with ctx.lock:
        if ctx.retire_deadline and now >= ctx.retire_deadline:
            _log("KME-KMS", f"{label} Commit: retired old port={ctx.retire_port} after grace")
            ctx.retire_deadline = None
            ctx.retire_port = None
            ctx.retire_key_id = None
...
def kme_tcp_tls_port_worker(st: KmeState, ctx: KmeServiceCtx, fixed_port: int, backend_host: str, backend_port: int, label: str) -> None:
    ...
    while not st.stop.is_set():
        ...
        try:
            ...
            _log("KME", f"{listener_tag} TLS listener up (key_ID={kid})")
            backend_connected = False

            while not st.stop.is_set():
                # If this port is no longer valid for this key (rotation/grace ended), retire the listener.
                ...
                for rsrc in rlist:
                    if rsrc is proc.stdout:
                        ...
                        if not backend_connected:
                            ...
                            # --- OPTIMIZATION: Disable Nagle's Algorithm for KME TCP socket ---
                            backend.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                            # ------------------------------------------------------------------

                            backend_connected = True
                            _log("KME-TCP", f"{listener_tag} connected backend {backend_host}:{backend_port}")
                        backend.sendall(data)
                    else:
                        data = backend.recv(4096)
                        if not data:
                            raise RuntimeError("Backend EOF")
                        proc.stdin.write(data)
                        proc.stdin.flush()

        except Exception as e:
            _log("KME-ERROR", f"{listener_tag} exception: {repr(e)}")
        finally:
            try:
                if backend is not None:
                    backend.close()
...
def kme_tcp_tls_listener(st: KmeState, ctx: KmeServiceCtx, backend_host: str, backend_port: int, label: str) -> None:
    # Run dedicated workers for both ports so active and retire (grace) listeners can overlap.
    workers = [
...
class KeySyncHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            obj = json.loads(raw.decode("utf-8", errors="replace").strip() or "{}")
        except Exception as e:
        ...
        if op == "prefetch":
            ...
            if kids:
                st.prefetch_q.put(kids)
                _log("KME-KMS", f"Prefetch request: n={len(kids)}, kme_buf={st.keys.size()} (target~{SAE_POOL_TARGET})")
        ...
        if op == "prepare":
            ...
            _log("KME-KMS", f"Prepare: service={svc} key_ID={kid} -> port={port}, kme_buf={st.keys.size()}")
        ...
        if op == "commit":
            ...
            with ctx.lock:
                ...
                # Keep old listener/key alive during grace for in-flight/late reconnects.
            ...
            _log("KME-KMS", f"Commit: service={svc} key_ID={kid} new={new_port} old={old_port} grace={grace}, kme_buf={st.keys.size()}")
...
def run_kme(tag: str) -> None:
    ...
    # NOTE: distinct backends per TCP line (your request)
    threading.Thread(
        target=kme_tcp_tls_listener,
        args=(st, st.tcp1, KME_TCP1_BACKEND_HOST, KME_TCP1_BACKEND_PORT, "TCP1"),
        daemon=True,
    ).start()
    threading.Thread(
        target=kme_tcp_tls_listener,
        args=(st, st.tcp2, KME_TCP2_BACKEND_HOST, KME_TCP2_BACKEND_PORT, "TCP2"),
        daemon=True,
    ).start()

    httpd = HTTPServer(("0.0.0.0", KEYSYNC_PORT), KeySyncHandler)
    httpd.kme_state = st  # type: ignore[attr-defined]
    _log("KME-KMS", f"KeySync HTTP listening on 0.0.0.0:{KEYSYNC_PORT} (POST /keys) (switch_tag={tag})")
...
def main() -> None:
    ...
    try:
        t1, t2, u = _parse_switch_tag(tag)
    except Exception as e:
        print(f"Bad switch_tag: {e}")
        raise SystemExit(2)

    _log("MAIN", f"Role={role.upper()}, switch_tag={tag} (tcp1={t1}, tcp2={t2}, udp={u})")
