#!/usr/bin/env python3
"""
PQC Proxy - UDP-only, one-way, PERSISTENT PQC-TLS tunnel (ML certs)
...
- KME deframes and forwards exact original datagrams (prevents "2988 bytes" merges).
...
def setup_environment():
    env = os.environ.copy()
    # OpenSSL config/lib paths - keep your defaults
...
def get_cert_paths(role, sig):
    ...
    ML-DSA layout:
        ...
        certs_ml/ca_cert.pem
        certs_ml/sae_mldsa.pem
        certs_ml/sae_mldsa_key.pem
        certs_ml/kme_mldsa.pem
        certs_ml/kme_mldsa_key.pem
    """
...
class PersistentTLSClient(object):
    ...
    def __init__(self, kem, sig, host, port):
        self.kem = kem
        self.sig = sig  # kept for CLI compatibility, not used for ML
        self.host = host
        self.port = port
    ...
    async def _connect(self):
        ...
        args = [
            ...
            "-groups", self.kem,         # e.g. X25519MLKEM768
            "-quiet",
            "-verify_quiet",
        ...
        try:
            self.proc = subprocess.Popen(
                ...
                stdin=sock_in,
                stdout=sock_in,
                stderr=subprocess.PIPE,   # capture for immediate-exit diagnosis
                env=self.env,
            )
    ...
    async def send_bytes(self, blob: bytes):
        async with self.lock:
            try:
                ...
                log("SAE-TLS", "Connect failed, dropping: %r" % (e,))
                return
            ...
            try:
                ...
                try:
                    ...
                    log("SAE-TLS", "Resend failed, dropping: %r" % (e2,))
                    self._cleanup()
...
class SAEUDPHandler(asyncio.DatagramProtocol):
    ...
    SAE:
      - Listens on UDP :2152
      - Enqueues datagrams
      - A single sender task frames and forwards over persistent TLS
    """
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # One loop = one TLS connection handled by s_server.
                while self.running:
                    try:
                    ...
                    for payload in datagrams:
                        # Forward exact original UDP datagram (no merging)
